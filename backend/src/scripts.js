...
// 从工作流结果保存剧本（前端在工作流完成后调用）
router.post('/save-from-workflow', authMiddleware, async (req, res) => {
  const { projectId, title, content, tokens, provider, modelName } = req.body;
  const userId = req.user.id;

  if (!projectId || !content) {
    return res.status(400).json({ message: '缺少 projectId 或 content' });
  }
  ...
  try {
    ...
    const existingScript = await queryOne('SELECT id FROM scripts WHERE project_id = ?', [projectId]);
    if (existingScript) {
      return res.status(400).json({ message: '该项目已有剧本' });
    }
    ...
    // 保存剧本
    await execute(
      'INSERT INTO scripts (user_id, project_id, title, content, model_provider, token_used) VALUES (?, ?, ?, ?, ?, ?)',
      [userId, projectId, title || null, content, provider || 'unknown', tokenCount]
    );
    const scriptId = await getLastInsertId();
    ...
    await execute(
      ...
      [userId, scriptId, '剧本生成', provider || 'unknown', 'standard', tokenCount, unitPrice, amount]
    );
    ...
    res.status(500).json({ message: '保存剧本失败：' + error.message });
  }
});
